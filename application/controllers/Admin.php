		...
		if($this->admin_model->is_admin()){
			...
			switch($type){
				...
				case "team":
					...
					if($query->num_rows()==1){
						$query = $this->db->query("select * from itsa_team where team_examid=".$this->db->escape($id)."");
							...
							case "update":
								...
								if ($this->form_validation->run() === FALSE){
									switch($this->input->post('action')){
										...
										break;
										case "delete":
											if($this->db->delete('itsa_team', array('team_examid' => $id,'team_id'=>$this->input->post('team_id')))){
									    		$data['message']['class']="blue";
